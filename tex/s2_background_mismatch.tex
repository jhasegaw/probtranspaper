...
\begin{center}
  ...
  \begin{tikzpicture}[
    ...
    \node[boxed] (n1) at (-0.75,1) {{\color{blue} Human talker modeled by a} pronunciation FST,\\$\rho(\phi|w)$} 
    edge[pre2](n0);
    \node[boxed] (n2) at (2.5,1) {{\color{blue} Human listener modeled by a} misperception FST,\\$\rho(\psi|\phi)$}
    edge[pre2] (n1);
    \node[boxed] (n3) at (5.75,1) {{\color{blue} Human transcriber modeled as a} phoneme-to-grapheme FST, $\rho(\lambda|\psi)$}
    edge[pre2] (n2);
